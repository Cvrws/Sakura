...
public class TextField extends Gui {
    ...
    public void drawTextBox() {
        if (this.isVisible()) {
            ...
            if (s.length() > 0) {
                ...
                j1 = this.fontRenderer.drawStringWithShadow(s1, (float) xPos, (float) yPos + 1, i);
            }
            ...
            if (s.length() > 0 && flag && j < s.length()) {
                j1 = this.fontRenderer.drawStringWithShadow(s.substring(j), (float) j1, (float) yPos + 1, i);
            }

            if (flag1) {
                if (flag2) {
                    Gui.drawRect(k1, yPos - 1, k1 + 1, yPos + 1 + this.fontRenderer.FONT_HEIGHT, -3092272);
                } else {
                    Gui.drawRect(k1, yPos - 1, k1 + 1, yPos + this.fontRenderer.FONT_HEIGHT, -3092272);
                }
            }

            if (k != j) {
                int l1 = xPos + this.fontRenderer.getStringWidth(s.substring(0, k));
                this.drawCursorVertical(k1, yPos - 1, l1 - 1, yPos + 1 + this.fontRenderer.FONT_HEIGHT);
            }
        }
    }
	...
	public GuiPageButtonList.GuiResponder getGuiResponder() {
		return guiResponder;
	}
