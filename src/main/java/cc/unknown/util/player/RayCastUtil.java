...
public final class RayCastUtil implements Accessor {
            ...
            for (double yPercent = 1; yPercent >= -1; yPercent -= 0.5) {
                for (double xPercent = 1; xPercent >= -1; xPercent -= 1) {
                    for (double zPercent = 1; zPercent >= -1; zPercent -= 1) {
                        Vector2f subRotations = RotationUtil.calculate(((IEntity) entity).getCustomPositionVector().add(
    ...
    public static MovingObjectPosition rayCast(final Vector2f rotation, final double range, final float expand, Entity entity) {
        ...
        if (entity != null && mc.theWorld != null) {
            objectMouseOver = ((IEntity) entity).rayTraceCustom(range, rotation.x, rotation.y);
            double d1 = range;
    ...
    public boolean overBlock(final Vector2f rotation, final EnumFacing enumFacing, final BlockPos pos, final boolean strict) {
        final MovingObjectPosition movingObjectPosition = ((IEntity) mc.thePlayer).rayTraceCustom(4.5f, rotation.x, rotation.y);
